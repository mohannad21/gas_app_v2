...
class TestBaselineBalances:
    def test_customer_a_baseline(self, client, shared_baseline):
        # Customer A starts with money=500 debt, 12kg=5 owed, 48kg=0
        _assert_balances(client, shared_baseline["customer_a_id"], money=500, cyl12=5, cyl48=0)

    def test_customer_b_baseline(self, client, shared_baseline):
        # Customer B starts with money=-200 (we owe them), 12kg=-3, 48kg=0
        _assert_balances(client, shared_baseline["customer_b_id"], money=-200, cyl12=-3, cyl48=0)

    def test_customer_c_baseline(self, client, shared_baseline):
        # Customer C starts at zero
        _assert_balances(client, shared_baseline["customer_c_id"], money=0, cyl12=0, cyl48=0)


# --- Replacement ─────────────────────────────────────────────────────────────

class TestReplacement:
    def test_replacement_12kg_partial_payment(self, client, shared_baseline):
        ...
        # money: 0 + (300 - 100) = 200
        # cyl12: 0 + (3 - 2) = 1
        _assert_balances(client, shared_baseline["customer_c_id"], money=200, cyl12=1, cyl48=0)

    def test_replacement_12kg_full_payment(self, client, shared_baseline):
    ...
    def test_replacement_48kg_partial_payment(self, client, shared_baseline):
        post_replacement(
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=300, cyl12=0, cyl48=1)

    def test_replacement_adds_to_existing_balance(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_a_id"], money=800, cyl12=6, cyl48=0)

    def test_replacement_does_not_affect_other_customer(self, client, shared_baseline):
        post_replacement(
            client, shared_baseline["customer_c_id"], shared_baseline["customer_c_system_12kg"],
...
class TestSellFull:
    def test_sell_full_12kg_partial_payment(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=200, cyl12=0, cyl48=0)

    def test_sell_full_48kg_partial_payment(self, client, shared_baseline):
        post_sell_full(
            client, shared_baseline["customer_c_id"], shared_baseline["customer_c_system_48kg"],
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=400, cyl12=0, cyl48=0)

    def test_sell_full_does_not_change_cylinder_balance(self, client, shared_baseline):
        post_sell_full(
            client, shared_baseline["customer_a_id"], shared_baseline["customer_a_system_12kg"],
        ...
        _assert_balances(client, shared_baseline["customer_a_id"], money=1000, cyl12=5, cyl48=0)


# --- Buy empty ───────────────────────────────────────────────────────────────

class TestBuyEmpty:
    def test_buy_empty_12kg_partial_payment(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=-60, cyl12=0, cyl48=0)

    def test_buy_empty_48kg_partial_payment(self, client, shared_baseline):
        post_buy_empty(
    ...
    def test_buy_empty_does_not_change_cylinder_balance(self, client, shared_baseline):
...
class TestPaymentFromCustomer:
    def test_payment_reduces_money_debt(self, client, shared_baseline):
        ...
        post_payment_from_customer(client, shared_baseline["customer_a_id"], amount=200, happened_at=at(DAY1))
        # money: 500 - 200 = 300; cylinders unchanged
        _assert_balances(client, shared_baseline["customer_a_id"], money=300, cyl12=5, cyl48=0)

    def test_payment_does_not_affect_cylinders(self, client, shared_baseline):
        post_payment_from_customer(client, shared_baseline["customer_a_id"], amount=500, happened_at=at(DAY1))
        # Fully paid — cyl12 still 5
        _assert_balances(client, shared_baseline["customer_a_id"], money=0, cyl12=5, cyl48=0)

    def test_payment_overpayment_goes_negative(self, client, shared_baseline):
        post_payment_from_customer(client, shared_baseline["customer_a_id"], amount=700, happened_at=at(DAY1))
        # money: 500 - 700 = -200 (we now owe them)
        _assert_balances(client, shared_baseline["customer_a_id"], money=-200, cyl12=5, cyl48=0)

    def test_payment_does_not_affect_other_customer(self, client, shared_baseline):
        post_payment_from_customer(client, shared_baseline["customer_a_id"], amount=500, happened_at=at(DAY1))
        _assert_balances(client, shared_baseline["customer_c_id"], money=0, cyl12=0, cyl48=0)
...
class TestPayoutToCustomer:
    def test_payout_increases_money_owed(self, client, shared_baseline):
        # We pay out 100 to customer C (zero balance) → we now owe them 100 → money = -100
        post_payout_to_customer(client, shared_baseline["customer_c_id"], amount=100, happened_at=at(DAY1))
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=100, cyl12=0, cyl48=0)

    def test_payout_does_not_affect_cylinders(self, client, shared_baseline):
        post_payout_to_customer(client, shared_baseline["customer_a_id"], amount=50, happened_at=at(DAY1))
        # money: 500 + 50 = 550; cyl12 unchanged
        _assert_balances(client, shared_baseline["customer_a_id"], money=550, cyl12=5, cyl48=0)
...
class TestReturnEmpties:
    def test_return_12kg_reduces_cylinder_debt(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_a_id"], money=500, cyl12=2, cyl48=0)
    ...
    def test_return_48kg_reduces_cylinder_debt(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=0, cyl12=0, cyl48=-2)
    ...
    def test_return_both_gas_types(self, client, shared_baseline):
        ...
        _assert_balances(client, shared_baseline["customer_c_id"], money=0, cyl12=2, cyl48=2)
    ...
    def test_return_does_not_affect_money(self, client, shared_baseline):
        ...
        # money stays 500
        _assert_balances(client, shared_baseline["customer_a_id"], money=500, cyl12=0, cyl48=0)


# --- Customer balance adjustment ─────────────────────────────────────────────

class TestCustomerBalanceAdjustment:
    def test_adjustment_adds_to_all_dimensions(self, client, shared_baseline):
        post_customer_balance_adjustment(
            client, shared_baseline["customer_c_id"],
            money_balance=300, cylinder_balance_12kg=2, cylinder_balance_48kg=1,
            happened_at=at(DAY1),
        )
        _assert_balances(client, shared_baseline["customer_c_id"], money=300, cyl12=2, cyl48=1)

    def test_adjustment_negative_values(self, client, shared_baseline):
        post_customer_balance_adjustment(
            client, shared_baseline["customer_a_id"],
        ...
        _assert_balances(client, shared_baseline["customer_a_id"], money=400, cyl12=3, cyl48=0)
