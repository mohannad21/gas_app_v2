...
import { useInitializeSystem } from "@/hooks/useSystemSettings";
...
type WizardState = {
  ...
  companyFull12: string;
  companyFull48: string;
  companyEmpty12: string;
  companyEmpty48: string;
...
export default function WelcomeScreen() {
  ...
  const steps: StepConfig[] = useMemo(
    () => [
      ...
      },
      {
        id: "company_full",
        title: "Company",
        question: "Does the company need to give you any full tanks?",
        explanation: "You already paid for them, but you have not picked them up from the plant yet.",
        type: "yesno",
        arrow: "up",
        fields: [
          { key: "companyFull12", label: "12kg full", placeholder: "0", gas: "12kg", unit: "count" },
          { key: "companyFull48", label: "48kg full", placeholder: "0", gas: "48kg", unit: "count" },
        ],
      },
      {
        id: "company_empty",
        title: "Company",
        question: "Do you need to return any empty tanks to the company?",
        explanation: "Empties sitting on your truck or in storage that belong to the gas company.",
        type: "yesno",
        arrow: "down",
        fields: [
          { key: "companyEmpty12", label: "12kg empty", placeholder: "0", gas: "12kg", unit: "count" },
          { key: "companyEmpty48", label: "48kg empty", placeholder: "0", gas: "48kg", unit: "count" },
        ],
      },
      {
  ...
  const clearFields = (fields?: StepField[]) => {
    if (!fields) return;
    setState((prev) => {
      const next = { ...prev };
  ...
  const summaryLines = useMemo(() => {
    ...
    if (count(state.companyFull12) || count(state.companyFull48)) {
      lines.push(
        `Company gives full: ${count(state.companyFull12)}x 12kg, ${count(state.companyFull48)}x 48kg`
      );
    }
    if (count(state.companyEmpty12) || count(state.companyEmpty48)) {
      lines.push(
        `Return empties: ${count(state.companyEmpty12)}x 12kg, ${count(state.companyEmpty48)}x 48kg`
      );
    }
  ...
  const handleFinish = async () => {
    try {
      const payload = {
        ...
        company_full_12kg: toNumber(state.companyFull12),
        company_full_48kg: toNumber(state.companyFull48),
        company_empty_12kg: toNumber(state.companyEmpty12),
        company_empty_48kg: toNumber(state.companyEmpty48),
      };
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.navRow}>
        {stepIndex > 0 ? (
          <Pressable style={({ pressed }) => [styles.navButton, styles.navSecondary, pressed && styles.pressed]} onPress={goBack}>
