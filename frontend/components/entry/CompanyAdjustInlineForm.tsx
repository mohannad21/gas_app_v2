...
export default function CompanyAdjustInlineForm({
  ...
  const [adjustDate, setAdjustDate] = useState(date || getTodayDate());
  ...
  const baselineDate = isEditing && adjustment ? toDateKey(adjustment.happened_at) : date || getTodayDate();
  const baselineTime = isEditing && adjustment ? getTimeHMSFromIso(adjustment.happened_at).slice(0, 5) : getNowTime();
  const baselineMoney = isEditing && adjustment ? adjustment.money_balance : currentMoney;
  const baseline12 = isEditing && adjustment ? adjustment.cylinder_balance_12 : current12;
  const baseline48 = isEditing && adjustment ? adjustment.cylinder_balance_48 : current48;
  const baselineNote = isEditing && adjustment ? adjustment.note ?? "" : "";
  const isSubmitting = createAdjustment.isPending || updateAdjustment.isPending;
  const saveBusy = pendingSaveAction !== null && isSubmitting;

  useEffect(() => {
    if (isEditing) {
      if (!adjustment || seedKey === `edit:${adjustment.id}`) return;
      const moneyBalance = deriveMoneyBalance(adjustment.money_balance ?? 0);
      const cyl12Balance = deriveCylinderBalance(adjustment.cylinder_balance_12 ?? 0);
      const cyl48Balance = deriveCylinderBalance(adjustment.cylinder_balance_48 ?? 0);
      setAdjustDate(toDateKey(adjustment.happened_at) || date || getTodayDate());
      setTime(getTimeHMSFromIso(adjustment.happened_at).slice(0, 5));
    ...
    setAdjustDate(date || getTodayDate());
    setTime(getNowTime());
    setMoneyState(moneyBalance.state);
    setMoneyAmount(moneyBalance.amount);
    setCyl12State(cyl12Balance.state);
    ...
    setNote("");
    setSeedKey("create");
  }, [adjustment, balancesQuery.data, date, isEditing, seedKey]);

  const saveDisabled = useMemo(() => {
    if (isSubmitting || !balancesQuery.isSuccess) return true;
    if (!isEditing) {
      return nextMoney === currentMoney && next12 === current12 && next48 === current48 && !note.trim();
    }
    return (
  ...
  const reseedFromBalances = async () => {
  ...
  const save = async (resetAfter = false) => {
    ...
    try {
      ...
      let savedId: string;
      if (adjustment?.id) {
        await updateAdjustment.mutateAsync({ id: adjustment.id, payload });
        savedId = adjustment.id;
      } else {
      ...
      if (resetAfter && !isEditing) {
        await reseedFromBalances();
        onSaveAndAddSuccess?.();
        return;
      }
      onSaveSuccess?.({
        effectiveAt: `${adjustDate}T${time}:00`,
        highlightId: savedId,
      });
    } catch (err) {
      ...
      Alert.alert(
        ...
        getUserFacingApiError(
          err,
          isEditing ? "Failed to update company balance adjustment." : "Failed to save company balance adjustment."
        )
      );
    } finally {
    ...
    <View style={styles.root}>
      {showHeader ? (
        <View style={styles.headerRow}>
          <Text style={styles.title}>{title ?? (isEditing ? "Edit Company Balance Adjustment" : "Adjust Company Balances")}</Text>
          {onClose ? (
